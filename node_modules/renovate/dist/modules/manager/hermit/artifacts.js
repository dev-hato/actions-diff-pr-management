...
async function updateArtifacts(update) {
    ...
    logger_1.logger.debug({ packageFileName }, `hermit.updateArtifacts()`);
    try {
...
async function getUpdateResult(packageFileName) {
    ...
    // handle added files
    const added = await (0, p_map_1.default)([...hermitChanges.created, ...hermitChanges.not_added], async (path) => {
    ...
    const modified = await (0, p_map_1.default)(hermitChanges.modified, async (path) => {
        const contents = await getContent(path);
        return [
    ...
    const renamed = await (0, p_map_1.default)(hermitChanges.renamed, async (renamed) => {
